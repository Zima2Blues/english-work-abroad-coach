...
- [x] 新设备只要具备 Python 3.9+ 或 uv，即可按文档完成安装。
- [x] Python 版本不满足时在修改环境前快速失败，并给出可执行修复命令。
- [x] 同一设备上的三个平台共享同一份用户数据库，skill 目录可以只读。
- [x] 不同设备能够通过 `export` 和 `import --merge` 手动迁移同一份学习状态。
- [x] 旧计划、签到和提醒日志可以无损、幂等迁移。
- [x] 计划开始前汇总、非法时长、非法天数和路径空格均有回归测试。
- [x] 三个平台共享文件漂移会导致本地验证和 CI 失败。
- [x] Python 3.9/3.12、Linux/Windows/macOS CI 通过。
- [x] 三个 skill validator 通过，平台元数据和提醒边界正确。
- [x] 发布 ZIP 不包含环境、缓存或个人数据。
- [x] README、SKILL.md、安装 reference 和实际命令一致。
- [x] Git 工作区在日常 `today/checkin/summary/reminder` 后不出现运行数据修改。
